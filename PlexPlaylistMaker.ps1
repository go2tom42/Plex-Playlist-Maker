...
Param(
    ...
    [alias("File")]
    [String]$CSVFILEtemp = ".\playlist.csv",
    [parameter(Mandatory = $false)]
    [alias("Title")]
    [String]$PlaylistName = "Marvel / MCU"
)
<# MUST CHANGE #>
...
function get-categoryList-Content {
    ...
    foreach ($item in $categoryList) {
        $Tempcategorytype = ($CSVFILE | Where-Object { $_.type -eq "Film" } | where-object { $_.category -eq $item.Title })
        $templist = @()
    ...
    foreach ($item in $categoryList) {
        $Tempcategorytype = ($CSVFILE | Where-Object { $_.type -eq "Show" } | where-object { $_.category -eq $item.Title }).PlexTitle | Select-Object -Unique
